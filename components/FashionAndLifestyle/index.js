...
const FashionAndStyle = () => {
    ...
    const [fashionAndStyleData, setFashionAndStyleData] = useState([]);
    ...
    useEffect(() => {
        ...
        fetch('https://api.2geda.net/store/outlet-items')
            ...
            .then(data => {
                setFashionAndStyleData(data.items.fashionAndStyle);
            })
            .catch(error => {
    ...
    return (
        <>
            <div style={{ margin: '20px', background: 'white', borderRadius: '10px', padding: '20px', }}>
                <h4 style={{ color: 'rgba(0 0 0 0.80)', marginBottom: '20px', }}>Fashion And Lifestyle</h4>
                <ProductBox
                    productData={fashionAndStyleData}
                />
            </div>
        </>
    );
